...
func compile(submit *submitT) int {
	...
	localMkdirCmd := exec.Command("mkdir", "tmp/"+submit.sessionID)
	localMkdirCmd.Stderr = &stderr
	err = localMkdirCmd.Run()
	if err != nil {
...
func tryTestcase(submit *submitT) int {
	...
	for i := 0; i < testcaseN; i++ {
		...
		err = exec.Command("docker", "cp", "361025b0d0c2:/cafecoderUsers/"+submit.sessionID+"/userStdout.txt", "tmp/"+submit.sessionID).Run()
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s\n", err)
			return -1
		}
		err = exec.Command("docker", "cp", "361025b0d0c2:/cafecoderUsers/"+submit.sessionID+"/userStderr.txt", "tmp/"+submit.sessionID).Run()
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s\n", err)
			return -1
		}
		err = exec.Command("docker", "cp", "361025b0d0c2:/cafecoderUsers/"+submit.sessionID+"/userTime.txt", "tmp/"+submit.sessionID).Run()
		if err != nil {
			fmt.Fprintf(os.Stderr, "%s\n", err)
			return -1
		}
